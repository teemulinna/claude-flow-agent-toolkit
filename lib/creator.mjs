...
export class AgentCreator {
    ...
    async create(options) {
        ...
        if (!/^[a-z][a-z0-9-]*$/.test(name)) {
            throw new Error('Agent name must be lowercase and only contain letters, numbers, and hyphens');
        }
        ...
        if (options.config) {
            ...
            if (options.config.tools && config.tools) {
                config.tools = {
                    ...config.tools,
    ...
    async createBatch(batchConfig) {
        const { agents = [], stopOnError = false } = batchConfig;
        const results = [];
     ...
     */
    listTemplates() {
        return ['basic', 'swarm-coordinator', 'github-integration', 'code-analyzer', 'test-runner'];
    }
    ...
    async createFromPrompt(options) {
        ...
        const capabilityKeywords = ['analyze', 'create', 'validate', 'fix', 'monitor', 'coordinate'];
        ...
        const toolKeywords = ['read', 'write', 'bash', 'git', 'npm'];
        toolKeywords.forEach(keyword => {
            if (prompt.toLowerCase().includes(keyword)) {
                tools.allowed.push(keyword);
            }
        });
